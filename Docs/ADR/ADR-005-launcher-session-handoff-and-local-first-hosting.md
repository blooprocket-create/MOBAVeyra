...
3. **Session handoff:** the game never receives a password or the launcher's login credentials. The launcher logs in, requests from the backend a **single-use launch code** that expires within seconds and is bound to the account and client build version, and starts the game with that code. The game redeems it for its own session. The code is short-lived and single-use because other local processes can read a command line. Development tooling obtains launch codes from the local backend the same way, so the game's login path is identical with or without the launcher.
...
5. **Builds and CI:** the author's Windows PC builds the source engine and, later, runs as a self-hosted GitHub Actions runner so pull requests are compiled and tested. Cloud agent sessions cannot compile Unreal and must say so rather than claim a build passed.
...
- **Passing a login token or credentials to the game:** rejected; a long-lived secret on a command line is readable by other processes.
