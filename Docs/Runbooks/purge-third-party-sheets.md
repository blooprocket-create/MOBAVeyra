...
**Status:** **executed 2026-09-20.** Steps 1–5 complete and verified against the live remote. **Step 6 (a GitHub Support cache-purge request for the legacy repository) is not needed while that repository stays private; it becomes required only if it is made public again.**
...
That command is the historical step for the rewritten legacy repository and is kept as run. The project has since moved; a fresh clone of the current repository comes from `https://github.com/blooprocket-create/MOBAVeyra.git`, which never carried the content.
...
- [x] Mark register §A complete. *(Done 2026-09-23 — §A and §G item 4 now record the purge as executed, with the legacy-repository condition above.)*
