...
- The live game supports text **Team Chat, default-on All Chat with a player-controlled off setting, and Party Chat**, plus friend DMs including during a match; optional post-match chat includes both teams. There is **no built-in voice chat**. Message muting and All Chat opt-out are user-facing delivery/composition preferences, not evidence of wrongdoing or permission to expose confidential messages. Chat restrictions, reports and staff evidence access remain separately authorized and audited. See Chat & Communication Bible.
