...
A **ride state** is a movement mode in which a Vanguard is carried by an owned **vehicle**, replacing ordinary locomotion and the Vanguard's basic abilities for its duration.

A ride state is not a Dash, a Blink, or forced movement. The rider remains under normal movement control.

Every ride state explicitly defines its entry, its movement behavior, its replacement ability set, its duration, and each of its exits.
...
While in a ride state:

- Movement Speed is **set** to a data-driven value rather than added to the rider's ordinary Movement Speed. The §23 soft caps therefore do not apply to it.
- Slows still apply, reducing from the set value, and §23's ordinary slowing floor still applies.
- The rider is **Ghosted** (§24). Ghosted is unit collision only; terrain is never ignored.
- A ride state grants **no terrain traversal**. It cannot cross terrain an unmounted Vanguard could not.
- The rider's **facing changes at a limited angular rate** rather than instantly. The rate does not scale with current Movement Speed.
- Pathing is otherwise ordinary. The rider remains a normal pathing agent and accepts normal movement orders.
- A destination requiring more turn than the rate allows is reached by continuing at speed through the widest permitted arc, **not** by slowing in order to pivot.

The angular constraint is owned and applied by the server. Clients mirror it for display and prediction and never resolve it independently.

### Presence

The rider's gameplay hitbox during a ride state is a **single combined volume** for rider and vehicle, larger than the rider's ordinary hitbox. §13's visual fidelity rule applies to it as to any other hitbox.

Rider and vehicle are not separately targetable.

A ride state does not change vision radius and grants no reveal.

### Actions

While in a ride state, the rider:

- **cannot basic-attack**. Attack-move resolves as an ordinary move order.
...
A ride state replaces the rider's basic abilities in their slots for its duration.

Mounted actions:

- are **Abilities** for damage tags (§2), On Ability Hit (§16), and Ability Haste (§21);
- are **not basic attacks**, and trigger neither On-Hit effects nor item procs;
...
Crowd control applies to a rider by the ordinary rules of §8 and §9.

**No crowd control ends a ride state.** A rider who is stunned, rooted, feared, charmed or displaced remains in the ride state and resumes when the effect ends.

Hard CC reduces the rider's effective movement to 0 under §23. The set Movement Speed creates no exception to this.
...
A ride state defines every exit explicitly. Exits are:

- a **dismount ability** within the replacement set;
- **duration expiry**;
- any **recast that separates rider from vehicle**;
- **death**.

On leaving, the rider retains Movement Speed and decays to ordinary Movement Speed across a defined window rather than changing instantly.

### The separated vehicle

Where an exit leaves the vehicle travelling without its rider, that vehicle is a **projectile** under §13. It is not a summon, companion or clone under §32.

A separated vehicle:

- has no Health and cannot be damaged or destroyed;
- is interceptable by Spell Shields (§19) and projectile interception (§20) under their ordinary rules;
- is stopped by terrain;
- passes through units, applying its effect **once per enemy per cast**;
- attributes its damage, kills and assists to the rider under §32, including where the rider is already dead.

A separated vehicle does **not** damage structures unless explicitly flagged structure-enabled under §33's second exception, which requires it to define its own structure ratio. Tower aggression follows the ordinary owner-attribution rule, including its owner-in-range condition, without exception.

Where death caused the separation, the vehicle travels along the rider's heading at the moment of death and accepts no further input.

### Tuning

Set Movement Speed, angular rate, combined hitbox size, ride duration, the post-exit decay window, entry and mounted cooldowns, any structure ratio, and every effect magnitude are data-driven values.

## 57. Combat-spec completion rule
...
## 58. Remaining open combat questions
