...
- Custom match replay coverage also includes **single-human 1v0 solo practice, uneven teams, mixed human/AI sides and individually configured bot difficulties**. Session start/stop, resulting match timeline and whether an open-ended solo session counts as a completed game for public retention require explicit custom/replay architecture design; never drop the recording merely because the match is nonstandard.
...
- The host of an **invite-only custom match** may invite spectator-only guests separately from Vanguard-bearing participants; those guests do **not** occupy either team's player slots, control a Vanguard or influence match results. The pre-game client admits authorized guests to Unreal **Spectator Mode** for that specific custom session.
