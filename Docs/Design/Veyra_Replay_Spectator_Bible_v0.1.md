...
See [ADR-003](../ADR/ADR-003-unified-unreal-client-states.md). Mode map/input/asset cleanup and return/navigation lifecycle remain implementation details, not a new replay or spectator permission. The pre-game client design is paused after Proposal **92**.
