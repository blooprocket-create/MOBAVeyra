...
**Related:** [Client & Platform](Veyra_Client_Platform_Bible_v0.1.md) owns the launcher handoff and client-state transitions; [Parties & Social](Veyra_Parties_Social_Matchmaking_Bible_v0.1.md) owns friendship, party membership, blocks and Appear Offline; [Moderation & Telemetry](Veyra_Moderation_Telemetry_Bible_v0.1.md) owns report eligibility, sanctions and restricted evidence; [Replay & Spectator](Veyra_Replay_Spectator_Bible_v0.1.md) owns public playback/chat separation; [Match Flow](Veyra_Match_Flow_Bible_v0.1.md) owns post-match lifecycle and live-match phases.
...
- The direct-message conversation remains available when the player returns from live gameplay to the ordinary client state. Receiving a message should be **non-obtrusive during combat**, not cover or steal control from an active teamfight.
...
- Individual mute and participant-based match reporting remain available. **Decided by UX-59–60 (see §10):** the post-match chat is opt-in by first message; a player's participation ends when they leave immediate results or enter another committed match, and saved Match History does not reopen it; sending a message cannot bypass All Chat Off. Recipient rules after a participant reconnects, and whether a player with All Chat Off can opt into post-match chat at all, remain open; do not silently treat the optional post-match space as archived/published conversation.
...
## 7. Settings checkpoint — chat presentation and voice boundary (2026-09-21)

The Settings & Accessibility Bible now records **Standard/Large/Extra Large independent chat text** (SET-66), **Transparent/Standard default/High Contrast in-match chat backdrop** (SET-67), and **optional ping-type text labels** (SET-68). These are player-local presentation choices; chat authorization, recipient scopes, permitted ping information and delivery are unchanged.

**No built-in voice chat** remains the explicit rule (SET-72 rejected); push-to-talk/microphone/voice-channel settings must not be introduced. SET-73 rejected a separate All Messages/DM Only/Off chat-sound classification feature; the earlier, general incoming-chat-message sound toggle (SET-23) still exists. SET-51 rejected an additional configurable Do Not Disturb During Matches mode; friend DMs must nonetheless remain non-obtrusive during combat per §4.


## 8. Pre-game client UX checkpoint — UX-1–17
...
If the application has closed or crashed while a match is still live, on restart it presents a dedicated **Reconnect-only** state. Reconnect is its **only available action**: that state offers no sidebar, Party Chat, DMs, notification center or normal page navigation. Existing party and conversation state remains preserved, and permitted chat in **reconnected live gameplay** remains available under the rules above. Normal pre-game social UI returns after the match has ended. This is a UI availability rule, not a change to chat permissions, recipients or message retention.

**Historical checkpoint (2026-09-23):** UX-33–34 approved the champion-select chat composer; the author subsequently continued past UX-49 and the **current pause is after UX-92**.


## 9. Champion-select compact chat refinement — Pre-game UX-33–34 (2026-09-23)
...
## 10. Post-match opt-in chat and unified application — UX-59–60, UX-90–92 (2026-09-23)
...
One installed Unreal application hosts ordinary client, champion select and live gameplay states ([ADR-004](../ADR/ADR-004-unified-unreal-client-states.md)). Party Chat and permitted DMs retain conversation continuity **across state changes/process restart** without implying access to ordinary pre-game chat UI during live-match Reconnect-only. In-match Unreal communication remains available when assigned gameplay resumes. Do not reintroduce a second background pre-game chat executable.

**Current Pre-Game Client UX pause: after UX-92.**


## 11. Deliberately open — later Chat UX and Settings Bible
...
- Post-match chat: lifetime and the no-bypass-of-All-Chat-Off rule are decided (§5, §10; UX-59–60); whether a player with All Chat Off can opt into post-match chat at all remains open; do **not** override a player's disabled All Chat preference by silently funneling opponent messages through another channel.
