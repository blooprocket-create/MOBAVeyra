...
**Related documents:** The [Modes & Access Bible v0.1](Veyra_Modes_Access_Bible_v0.1.md) owns mode access and Co-op vs AI composition; the [Custom Matches Bible v0.1](Veyra_Custom_Matches_Bible_v0.1.md) owns host-configured practice, optional victory rules and manual custom-session ending; the [Parties & Social Bible v0.1](Veyra_Parties_Social_Matchmaking_Bible_v0.1.md) owns pre-select acceptance, parties and queue restrictions; the [Client & Platform Bible v0.1](Veyra_Client_Platform_Bible_v0.1.md) owns the launcher handoff, client-state transitions and reconnect UX; the [Replay Bible v0.1](Veyra_Replay_Spectator_Bible_v0.1.md) owns match replay coverage and spectator delay. The [Chat & Communication Bible v0.1](Veyra_Chat_Communication_Bible_v0.1.md) owns live/post-match text channels, including default-on All Chat and persistent Party Chat. The Battleground Bible owns map structures, wave/camp schedules, and the Prime Well win condition. The Combat Bible owns damage, actual death, and control/target validity. The Economy & Progression Bible owns Gold, XP, and buyback. The Vision Bible owns vision and ward tools.
...
2. **Loading:** Wait for all **required human player connections** up to a **configurable loading timeout** (prototype starting value **90 seconds**; ten for PvP; five human teammates for Co-op vs AI, with five server-controlled enemy AI Vanguards).
3. **Fountain preparation:** Players enter a synchronized **15-second prototype preparation period**. They can move **within their own fountain**, buy opening items, and allocate starting skill points, but **cannot leave the fountain**. This preparation countdown happens before the match clock starts.
...
- **Every match produces a gameplay replay**, including games that end in remake, under the Replay Bible's recording rule (Replay Bible §1, the owner of recording coverage; coverage for remade, abandoned or crashed sessions is still listed as replay architecture work in Replay Bible §9); an invalidated/no-contest match has a replay but still awards no account XP under the Account Bible.
- **Custom practice may disable normal victory conditions**; its host can manually **End Custom Match**, producing an explicitly host-ended noncompetitive completion and a replay. **Solo 1v0 cannot auto-win at launch for lack of opposition.** This is a custom-only exception, not permission to change matchmade victory adjudication, force a team win/loss, or award account XP/Mastery; see Custom Matches Bible for the remaining lifecycle details.
...
- **Post-match chat is an optional cross-team results-screen conversation**, never a gate to leave, return to party or requeue. Party Chat continues across pre-game, select, live match and post-match; friend DMs may be answered during the match. Team/All Chat, default-on All Chat toggle and individual mute behavior are specified in the Chat & Communication Bible. Public replay/spectator UI remains chat-free.


## 13. Loading-screen presentation checkpoint (SET-111–120; 2026-09-21)

The [Settings & Accessibility Bible](Veyra_Settings_Accessibility_Bible_v0.1.md) owns loading-screen presentation, not the authoritative match timeline. **Required graphics preparation** and any readable error/retry feedback appear on the **match loading screen**; an affected player does not enter live gameplay while required preparation is incomplete. Apply this within this bible's **existing tunable loading timeout, no-show and connection rules**—do not hold everyone indefinitely or invent a new match-start rule.

Show plain-language loading-stage labels and honest indeterminate progress, honor visual accessibility preferences and preserve actionable errors. Optional gameplay tips/lore facts have **Both (default), Tips Only, Lore Only or Off** selection, manual Previous/Next, varied non-repeating automatic rotation, and **at least 8 seconds** of automatic display per entry (longer for longer text). These presentation timings **never delay completion of loading or match entry**. No new loading audio cue is approved. See SET-111–120 for detailed boundaries.


## 14. Pre-game client selection and reconnect presentation checkpoint (2026-09-22)

The [Pre-Game Client UX Bible](Veyra_Pre_Game_Client_UX_Bible_v0.1.md) first documented UX through UX-17; that is a **historical** checkpoint. The latest author-approved client state/transition design is through **UX-92**, now paused. This section is a **presentation cross-reference**; match cancellation, dodge, reconnect, absence and outcomes remain authoritative under the preceding rules.

- **Match Found:** Prominent blocking Accept/Decline overlay prevents ordinary client navigation and other actions while authoritative match acceptance is pending. After acceptance, show waiting for remaining participants; start champion select only after the existing match-assembly confirmation.
- **Champion select:** Takes over the entire pre-game client as the committed opening match stage. Ordinary browsing, shopping, Settings, party management and other pre-game pages are unavailable; the party UI hides and friends/chat auto-minimizes, while permitted selection-safe chat may be reopened without allowing navigation away. On cancellation, return to an eligible normal pre-game view under the existing disconnect/dodge/party rules. This UI restriction does not create an extra match phase or new dodge penalty.
- **Unexpected application exit while live match remains:** On restart the application displays only its dedicated **Reconnect-only** state. **Reconnect is its sole available player action**; no pre-game shopping, account settings, social messaging, party controls or new queue. Reconnect returns the player to **the same assigned Vanguard and live match** and does not reset absence/penalty state. The party and other trusted state remain preserved; normal permitted in-match chat works again in live gameplay after reconnect. These restrictions govern client UI and **do not** alter match server timers, ongoing match state or OS process behavior.
- **Verified match completion:** Reconnect-only view ends and the pre-game client displays post-match results as primary content; clearly mark any pending trusted reward processing. Normal party/social surfaces resume and retained party members return to Not Ready while preserving an eligible selected mode. Other members may browse or prepare the next queue without waiting for everyone to dismiss results.

**Updated checkpoint (2026-09-23, historical):** At that checkpoint the pre-game client discussion had approved decisions through **UX-49**, then paused. Superseded — the discussion resumed and the current pause is after UX-92 (see §16). The earlier UX-17 checkpoint above is also historical.


## 15. Pre-game client selection and results continuation — UX-39–49 (2026-09-23)

During an interrupted champion-select connection, the UI may attempt to rejoin the **same active server-authoritative selection session**, restoring only confirmed picks/bans, assigned Vanguard, selected skin, starting Flux Spells, phase and remaining time. Existing deadlines, timeout/dodge outcomes and match authority continue unchanged; an expired session displays its actual outcome. A confirmed successful selection transitions through truthful Match Starting/Preparing Gameplay/Connecting states **within the same Unreal application**, then assigned live gameplay; failure or process restart during an assigned live match remains **Reconnect-only** without a second ready check or new queue. Older references to launching a second Unreal executable are superseded by client UX-91–92.

On verified match completion, present trusted post-match outcome, progression and the approved match-statistics record. Statistics and final equipment are server-confirmed, not inferred by pre-game UI; pending fields remain honestly pending. The detailed results-page layout was deferred at UX-41 and was not silently approved by the subsequent statistics decisions; **it was later approved in UX-50** (Pre-Game Client UX Bible §7). Statistics authority/definitions and the UX-47 rejection are documented in the Pre-Game Client UX Bible §§5–6 and Match Statistics Bible. Individual loss overrides and no-contest rules in this Match Flow Bible retain precedence over any shorthand team win/loss field in a scoreboard.


## 16. Unified Unreal match-state transition checkpoint — UX-90–92 (2026-09-23)

**One installed Unreal application**, not a separate pre-game process launching a second game executable, hosts ordinary client, Test Skin, Match Found, champion select, assigned live gameplay, verified results, Replay/Spectator and Reconnect-only **as isolated states**. See [ADR-004](../ADR/ADR-004-unified-unreal-client-states.md) and the Client & Platform Bible. The launcher still handles authentication/install/patch; the dedicated match server still owns match truth, assignment, timing, outcomes and disconnect/AFK penalties. Do not infer that client state transitions pause the authoritative match or grant a new action to an absent player.

- Match Found immediately interrupts Test Skin; its optional preview loads and client cache preloading must not delay authoritative Accept/Decline or select. Accepted assembly closes Test Skin before committed select. Failed assembly returns to eligible ordinary state without inventing a party/queue result.
- After successful select, show truthful Match Starting → Preparing/Loading Gameplay → Connecting to Match, then enter controllable live match **only on confirmed assignment/server connection and required assets ready**. These are in-process states; **“Launching Game” in older documentation was superseded**. Existing match loading/preparation timeouts and no-show rules remain authoritative; do not hold a match indefinitely for optional assets.
- When the same app crashes/exits during an assigned live match or fails connection while match is live, on return show **Reconnect as the only pre-game action**, bound to that match/Vanguard. No shopping, socials, test preview, fresh queue, cancellation of server absence clocks, or new login/selection route to escape. On authoritative completed-match confirmation, transition to results and restore ordinary client shell under existing party/readiness rules.
- State/map cleanup, source-asset caching and UI lifecycle must not introduce a client-side victory, remake, penalty, payment, grant or stopwatch. Run tests for transitions during test asset load, session loss and process restart.

**Current Pre-Game Client UX pause: after UX-92; no UX-93 until author “continue”.**


## 17. Tuning, tests, and deliberately open questions
...
**Still open—not to be improvised as canon:** precise activity/contribution anti-abuse criteria; detailed queue-dodge penalties; how reconnect/no-show behavior interacts with the pre-match loading/preparation boundary; pause/resume UI and early-resume voting timeout; concurrency rules for simultaneous pause/remake/surrender requests; and persistence/rating calculations for individual losses, forgiveness, and no-contest remakes. These choices can be specified when the first playable and match service require them.
