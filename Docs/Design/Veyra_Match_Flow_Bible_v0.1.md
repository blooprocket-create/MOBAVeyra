...
## 13. Loading-screen presentation checkpoint (Settings proposals 111–120; 2026-09-21)
...
Show plain-language loading-stage labels and honest indeterminate progress, honor visual accessibility preferences and preserve actionable errors. Optional gameplay tips/lore facts have **Both (default), Tips Only, Lore Only or Off** selection, manual Previous/Next, varied non-repeating automatic rotation, and **at least 8 seconds** of automatic display per entry (longer for longer text). These presentation timings **never delay completion of loading or match entry**. No new loading audio cue is approved. See Settings Proposals 111–120 for detailed boundaries.


## 14. Pre-game client selection and reconnect presentation checkpoint (2026-09-22)

The [Pre-Game Client UX Bible](Veyra_Pre_Game_Client_UX_Bible_v0.1.md) first documented UX through Proposal 17; that is a **historical** checkpoint. The latest author-approved client state/transition design is through **Proposal 92**, now paused. This section is a **presentation cross-reference**; match cancellation, dodge, reconnect, absence and outcomes remain authoritative under the preceding rules.
...
**Updated checkpoint (2026-09-23, historical):** At that checkpoint the pre-game client discussion had approved decisions through **Proposal 49**, then paused. Superseded — the discussion resumed and the current pause is after UX Proposal 92 (see §16). The earlier Proposal 17 checkpoint above is also historical.


## 15. Pre-game client selection and results continuation — Proposals 39–49 (2026-09-23)

During an interrupted champion-select connection, the UI may attempt to rejoin the **same active server-authoritative selection session**, restoring only confirmed picks/bans, assigned Vanguard, selected skin, starting Flux Spells, phase and remaining time. Existing deadlines, timeout/dodge outcomes and match authority continue unchanged; an expired session displays its actual outcome. A confirmed successful selection transitions through truthful Match Starting/Preparing Gameplay/Connecting states **within the same Unreal application**, then assigned live gameplay; failure or process restart during an assigned live match remains **Reconnect-only** without a second ready check or new queue. Older references to launching a second Unreal executable are superseded by client UX Proposals 91–92.

On verified match completion, present trusted post-match outcome, progression and the approved match-statistics record. Statistics and final equipment are server-confirmed, not inferred by pre-game UI; pending fields remain honestly pending. The detailed results-page layout was deferred at UX Proposal 41 and was not silently approved by the subsequent statistics decisions; **it was later approved in UX Proposal 50** (Pre-Game Client UX Bible §7). Statistics authority/definitions and the 47 rejection are documented in the Pre-Game Client UX Bible §§5–6 and Match Statistics Bible. Individual loss overrides and no-contest rules in this Match Flow Bible retain precedence over any shorthand team win/loss field in a scoreboard.


## 16. Unified Unreal match-state transition checkpoint — Proposals 90–92 (2026-09-23)
...
**Current Pre-Game Client UX pause: after Proposal 92; no Proposal 93 until author “continue”.**
