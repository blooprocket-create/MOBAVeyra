...
- Guaranteeing *zero consecutive-week repeats* for 12 slots requires **at least 24 release-eligible distinct Vanguards**. A hypothetical **25-Vanguard release-eligible pool** can support a zero-repeat 12-slot weekly rotation if at least 24 are actually eligible after the new-release waiting period. If fewer than 24 are eligible, the established repeat-allowing fallback continues to apply. Twenty-five **designed** Vanguards do not necessarily mean twenty-five released or rotation-eligible choices.
...
- Custom hosts may **disable normal victory conditions** for open-ended practice and conclude the session using **End Custom Match** with no competitive winner; **solo 1v0 defaults to open-ended practice rather than an instant victory**. Exact custom host-configurable options, draft/mirror settings, result/scoreboard details and custom-lobby lifecycle remain open in the Custom Matches Bible. Invited spectator-only guests and a host-selected three-minute or zero-delay feed are defined there. Human selection is owned-or-rotation; bot Vanguard selection is any released Vanguard. Custom games grant no Account XP/Mastery.
