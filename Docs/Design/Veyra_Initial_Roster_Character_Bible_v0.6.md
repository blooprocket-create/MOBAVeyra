...
**Appearance pass (2026-09-20):** `**Visual language:**` paragraphs written for the fourteen Vanguards that had a surviving character sheet but no appearance description in canon (Silt, Relay, Patch, Tavi, Vera, Korruk, Torr, Marek, Neris, Moro, Qazharr, Varkesh, Sylra, Mavra), read off that existing art so the design survives regeneration. Raska, Kade and Angeru were deliberately excluded pending a clean redesign (since completed — see **Clean redesign** below); Vanguards 21–25 had no art at the time and have since received authored hero art. No kit, lore, role, region or roster count changed.  
**Art precedence (2026-09-20):** **Approved artwork supplied by the author is canon.** Where a `**Visual language:**` paragraph, a header field or any other description disagrees with approved art, the art wins and the text is corrected to match — including its anti-drift guardrails, since a guardrail written for a superseded design steers every remaining slot wrong. Text still owns everything an image cannot carry: what the design is *not*, gameplay-relevant figures, and the reasons behind a choice.  
...
**Art precedence applied beyond appearance (2026-09-20):** Vera's weapon identity changed from "repeating warbow/arc-crossbow" to a two-handed Flux-driven **Merrin siege arm**, because her approved art shows one and art is canon. This is the first time the precedence rule has corrected a line outside a `**Visual language:**` paragraph, so it is recorded here rather than left to a diff. Her kit is unaffected — Cadence, Range Found, Dig In, Break the Line and The Last Volley describe a firing weapon and a firing stance, never a bowstring. No kit, role, region or roster count changed.  
**Neris restored to the wave (2026-09-21):** A previous note here withdrew *"she rides a living wave rather than standing on the ground"* because her first authored art put her on wet rock. That art is being replaced: it had converged with Sylra's almost exactly — same hood, same ribboned grey-blue robe, same lit lantern on chains in the right hand, same ship's-wheel charm — and the author has chosen to keep Sylra and move Neris. **The wave is restored as her defining silhouette**, which is also her Q, her core fantasy and her Wakebound nature, so the separation costs nothing and returns something. The **lantern and the ship's-wheel charm are given to Sylra outright**: Sylra's lantern is load-bearing (*"the point allies steer by"*) and Neris's was only ever listed among working gear. Her hat stays a brimmed hat rather than a hood, which is what canon said before either image existed. No kit, role, region or roster count changed.  
**Moro's Wildlight recoloured (2026-09-20):** Gold-green Wildlight and yellow-gold eyes are withdrawn; his approved art paints both **blue-violet**, and art is canon. **This has a cost worth stating.** The World Bible calls Wildlight *"natural magic distinct from Flux technology"*, and colour was what carried that distinction — Moro's markings now sit in the same band as Relay's and Torr's Flux light. What still separates them in the art is **form**: Wildlight grows organically along limb and grain, while Flux is geometric glyphwork on stone and machine. That is now written into his guardrails, because it is the only thing left doing the job. **Confirmed by the author (2026-09-20): the art is what they want**, so blue-violet is settled and gold-green is closed. No kit, role, region or roster count changed.  
**Scale is not the bible's to fix (2026-09-21):** Relay's and Torr's entries carried “an exact metre figure is still to be set”, which was the wrong obligation in the wrong document. Two project rules settle it. **Combat Bible §"Gameplay hitboxes":** *"Gameplay collision/hurtboxes are authoritative, not the visual model"* — so a character's collision size is deliberately independent of how tall the art makes them look, and no metre value read off an illustration would govern it anyway. **`ARCHITECTURE.md` §1.3:** distances, radii and caps are tuning, owned by validated designer-editable data, and *"temporary prototype numbers belong in data too"*. That data asset does not exist yet, because no project does. So the bible records **visual scale as art direction** — which is what a modeller and a prompt need — and the capsule figure waits for the owning data layer and a grey-box, where lane width and camera height decide it rather than lore. Author's ruling, 2026-09-21: sizes are **large**, exact values unknown, which is a complete answer at this layer.  
**Angeru's staging corrected from art (2026-09-21):** His approved art shows him **crouched on a rooftop under dramatic night light**, sword across his back. His paragraph specified the opposite staging — *"calmly and evenly lit, standing straight… **not** crouched"*, sword *"at the hip"* — so that staging is withdrawn and the description follows the art. **His characterisation is untouched.** The staging existed to make him read as methodical rather than as a generic edgy assassin, and what does that job in the art is his **face**: flat, level, no snarl, uncovered. The guardrail is rebuilt around the face instead of the pose, so it still forbids the wrong reading. The antagonist direction in his `**Visual language:**` paragraph loses only its staging clause; *composed, methodical, unhurried, explains himself, takes no pleasure* all stand. The art is **silent** on the cut-out insignia rather than contradicting it, so that detail stays — canon calls it the character. No kit, lore, role, region or roster count changed.  
**Eudora's prostheses extended from art (2026-09-21):** Canon specified a single **mechanical left-forearm prosthesis**. Her approved art gives her **both arms from the shoulder and both legs below the knee** — four limbs rather than one forearm — so both her Nature and Visual fields are corrected to match. **This is the largest single change the art has made to a character.** It left a lore question the art could not answer — one forearm needs no explanation for an engineer, four limbs implies a specific catastrophe — and the author has ruled that the lore should follow the image. **Resolved 2026-09-21:** a freight tunnel came down on a crew she was leading. The injury now sits *before* Picket in her history, so the machine reads as her answer to it: she had spent a shift under a ceiling with nothing over her, and built mobile cover so the next crew would have something. The night-shift attack stays a separate, later event — that is when the rivet cannon was fitted. No blame is assigned to an employer; her existing hostility to negligent ones does that work already. She remains human: torso, neck and head are flesh. No kit, role, region or roster count changed.  
**Tavi's cross pupils, twice unmet (2026-09-21):** Her paragraph specifies *"Her pupils are **pink and cross-shaped**"*. Two successive authored heroes have given her round pink pupils instead, while delivering the cross mark emphatically everywhere else. The spec is **kept rather than withdrawn**: the crosses elsewhere show the idea is landing, and a detail a generator declines twice is a production limit rather than a design decision. **If a third image declines it, withdraw it** — canon should not hold a line no image will satisfy, and the mark survives without it. No kit, lore, role, region or roster count changed.  
...
**Scale:** **Large** — machine-scale, well above human height, standing among ruins of roughly his own size. The earlier “roughly 3.8m” stays withdrawn: the art shows something clearly bigger. **Corrected 2026-09-21:** my replacement wording, “he towers over the ruins around him”, was also an over-read — the arches beside him reach his head and the towers behind him are taller. His art carries **no reliable scale reference** at all: no figure, no doorway, no standard object, and foreground flowers that are close to the lens rather than large. “Large” is as far as this image can be read, and that is enough (see **Scale is not the bible's to fix** above).  
...
**Scale:** **Colossal**, and the largest Vanguard on the roster. His art is the only one carrying a usable reference — a cloaked human stands on rubble at the lower left, measuring 7.3% of the frame height against a figure that spans nearly all of it, so he reads at roughly **ten times a person**. A low wide-angle lens inflates the nearer subject, so treat that as an upper bound on the intended *visual* scale rather than a measurement. R (Overcapacity) enlarges him further on top of it.  
...
**Visual language:** **She rides a living wave** — that is the silhouette, and everything else is secondary to it. A standing swell of dark water carries her above the surface, curling and breaking as it moves, and she stands braced on it with her weight forward like someone who has done this a thousand times. Long pale blue-white hair streams back. A **dark wide-brimmed hat**, a heavy layered coat gone ragged at the hem, and a working harness of chains, rope and salvage gear — a heavy shackle at the chest, a coil of line at the hip, a metal signal whistle on a chain. Her hands are bare and open, and the water answers them. Her **two sea states** show in the wave rather than in her — **Calm** is a low steady swell, soft light and an open silhouette; **Storm** raises the surge under her, lifts her higher, darkens the palette and turns it aggressive. Behind her the Drowned Cantons at night: ships at anchor, waterfalls falling through ruined arches, harbour lamps burning small and far off. She is a rescuer who drowned and came back still working, and the sea is the thing she rides rather than a thing she haunts. Sylra carries the lantern and the ship's-wheel charm; Neris is told apart from her by the wave, by open water instead of a dock, and by a brimmed hat instead of a hood. She carries **no** lantern and **no** ship's-wheel charm. She is **never** a drowned ghost, and the water is her mount and her tool, **not** a monster she commands.
...
**Visual language:** A large quadruped jungle predator with a dark bark-and-fur body — weathered grey timber through the haunches and shoulders, thick cream-white fur at the ruff and chest — and branching antlers grown through with living wood. **Wildlight** markings run along his limbs and flanks in **blue-violet**: leaf-shaped and eye-shaped panels, veining that follows the grain of him, brightening visibly as he channels. His eyes burn the same blue. He carries moss and small growing things on his back, as though the forest has begun using him for ground. He is genuinely an animal: no armour, no saddle, no harness, no handler, no speaking humanoid hidden inside the beast design. He is intelligent and territorial, and that has to read through posture, attention and stillness rather than through anything worn or carried. Wildlight is **never** the geometric glyphwork of Flux — it grows along limb and grain like veining in a leaf. Colour no longer separates the two, so shape has to.
...
**Visual language:** A roughly 3m figure — close to twice a person's height — built from dark interlocking iron plates over a molten interior, with a bright orange-white **core spiral** set into the chest. The seams between plates glow, and movement sheds sparks and molten fragments: plates at his extremities ride slightly clear of the body, and broken pieces hang in the air around a raised hand. A heavy cloth drape hangs at the waist, held by iron rings. His head is a faceless wedge of plate set low between enormous shoulders, crested with short spines and lit from inside like the rest of him. **Both hands stay open and empty**, fingers spread. Molten material tears loose from his plating and orbits them, ready to be thrown; a strip of ground has already gone molten where he is working, and at distance the leading edge of it is cooling into black iron. That is his whole offence and his silhouette has to say so — everything he throws is drawn off his own body. Behind him the Ember Basin: lava running in channels through a burning ruined city. **The metal is his body, not armour worn over something else**: never draw a person, pilot, or face inside the shell. He carries **no** weapon — never a sword, never an axe, never anything with a hilt or a forged edge. He is a Forgeheart — an elemental consciousness that formed in the Ember Basin crucibles — **not** a golem, a construct, a Fluxborn, or a transformed human.
...
**Visual language:** He wears the equipment of both houses at once, mixed and deliberately stripped of its marks. From the **Blade House**, structured formal pieces — disciplined lines and the long single-edged sword, carried sheathed across his back. From the **Veil House**, soft dark wrapping — forearm and shin bindings, a hood down around his shoulders, cloth cut to move silently, and the thin throwing needles of Shadow Needle. A torn red sash and cloak-tail trail behind him, the one loud colour on an otherwise black figure. A round **crossed-blades token** hangs at his belt beside a drawn dagger. **Every house insignia has been cut out of the cloth, leaving clean empty patches where the marks were**, and that detail is the character: he took both disciplines and belongs to neither. His face is uncovered. Concealment is a tool he uses rather than an identity he keeps, and he intends to be recognised. When **False Body** is active, a featureless dark double stands apart from him holding the same posture. Behind him the Reed Provinces: terraced fortress towns and shrines stacked up a mountainside, red banners, narrow bridges over cloud, a low moon. What carries him is his **face**, which stays flat, level and unhurried whatever his body is doing. He is **never** snarling, **never** masked, and **never** theatrical. The menace is that he is perfectly calm and will explain his reasoning while he works.
...
**Skill points (ruled 2026-09-25).** Forsake the Schools costs no skill point and never takes ranks. Each skill point spent in Q, W or E ranks **both** stances' abilities in that slot together: Shadow Needle and Flowing Cut share the Q rank, False Body and Severing Arc share the W rank, and Black Step and Passing Step share the E rank. Q, W and E each rank to **6** instead of 5, so Angeru spends the same 18 skill points as a standard kit. This is a documented exception to the Economy & Progression Bible's standard 5 + 5 + 5 + 3 ranks.
...
**Visual language:** A hooded harbour pilot in layered grey-blue storm cloth, cut into long ribbons at every hem so her edges move with the air. Dark markings run across her eyes, and the hood is deep enough to hold most of her face in shadow. A heavy chained coat hangs with working navigational gear — tuned bells, sea-glass, keys, small weights, a ship's-wheel charm. She carries a **large lit lantern** out at arm's length on its chain, burning cold blue-white: the brightest thing in any frame she is in, and the point allies steer by. Mist gathers around her and moves with her rather than drifting past. Behind her the Drowned Cantons at night — ships at their moorings, waterfalls off the cliff terraces, harbour lamps burning. Her silhouette is deliberately soft-edged and wrapped, difficult to resolve cleanly at distance; that ambiguity is the character rather than a readability failure, and the lantern is what fixes her position. **She and Neris share a region and a palette, so her silhouette has to stay separable from Neris's at gameplay distance.** Every object on her is functional wayfinding equipment. She is a pilot and a guide: **never** an occultist, and the gear is **never** ornament.
...
**Visual language:** An Iron March salvage chemist with dark red-brown hair pushed back under a red band, **goggles pushed up on her forehead**, and a heavy layered coat with red lining, its ragged hem painted with red-and-white hazard striping. Belts of canisters, sealed bottles and reagent hardware cross her body, and she carries a large **pressurised dispenser rig**: a banded steel cylinder with a glass window onto the glowing orange reagent inside, a lozenge hazard mark etched on the glass, a yellow-and-black placard on the pump housing, and armoured hoses running off it, one of them lit from within. Her hands and coat carry grime, burn marks and chemical staining from actual work. Behind her Calder's Run at work: gantries, stacks and pipe runs under smoke, molten runoff pouring from the outfalls. Her equipment is **industrial** rather than arcane: labelled containers, valves, straps, gauges and pressure fittings. It is **never** a witch's potion belt, and she should read as someone who fixes hazardous sites for a living rather than as an alchemist or a poisoner.
...
Tavi, Vera and Oriel are all Echoes, but their identities emerge from different repeated experiences:
...
**Visual language:** A gigantic, visibly **asymmetrical** mass of irregular riverstone, collapsed masonry, exposed moss, embedded weathered iron, and broken chains. His outsized hooked arm / chain apparatus dominates his silhouette; the other arm is shorter and built for support and ground slams. The hook is a single **rusted iron crescent** the size of his own head, hung on heavy chain that wraps his torso and shoulders and trails behind him into the water. Water runs off him constantly, sheeting from every ledge and seam, because he has only just come up out of the river. Moss and river growth hold the upper surfaces. The head is a small blocky mass set low between enormous shoulders, faceless. Avoid mirrored rock plates, uniform limbs, a smooth humanoid armor suit, or a symmetrical hook. His weight and silhouette should read clearly from gameplay camera distance. He is the river's own stonework roused: **never** a Fluxborn construct, and **never** lit from within. Behind him the Buried Riverlands: aqueduct arches and bridge ruins stepped down a gorge, waterfalls coming through them. Ancient bridge-ruin motifs tie him to the Buried Riverlands rather than a nautical harbor.
...
**Visual language:** A compact, visibly veteran gunner with short salt-and-pepper hair, a weathered/scarred face, a worn dark-teal naval coat, functional reinforcement gear, ammunition pouches, and a giant **asymmetrically carried** brass-and-iron cannon. Exposed blue-lit Flux chambers, stabilizing braces, and mechanical recoil hardware make the cannon immediately recognizable in silhouette and at in-game scale. **Mournwake's weight is carried on both sides of her**: the banded barrel runs forward under one arm to a wide muzzle with Flux light standing in the bore, and the breech and recoil assembly rides up over the opposite shoulder to balance it. Rope coils and a red sash sit at her waist among the buckles. Behind her the Cantons at night — ships at their moorings, lantern-lit terraces, water breaking against the pilings. She is a fighter with a giant engineered gun, **not** a fast-firing minigun operator: one heavy piece that has to be chambered, **never** a rotating barrel cluster or a belt feed.
...
**Visual language:** An elegant but unmistakably **nonhuman, asymmetrical** figure built from irregular stained-glass panes, fractured lead/metal framing, floating shards, and a warm luminous core. Her limbs are framed glass the whole way through. They **never** read as ordinary human skin under glass armour. She is memory held in glass: **never** a ghost, and **never** Flux-powered. Gold, pale blue, violet, and jewel-colored refractions intensify visibly with spellcasting. A **ring of gold framework** stands behind her head with the core burning at its centre, and broad glass panes fan from her shoulders like a window opened out. Below the waist she narrows to a trailing point of shards rather than legs, which is how the hovering reads: she floats clear of the ground, fragile, ancient and powerful rather than heavily armored. Behind her Merrin in full sun — cathedral ruins, flowering growth on the masonry, water falling through the arches.
...
**Visual language:** Oversized ears, fluffy tail, short muzzle, paw-like hands with opposable thumbs, digitigrade feet, enormous teal hood, long scarf, trinket satchel, and small floating gold-and-blue rings firing star-like bolts. The **Tinkertwins** orbit her hands rather than being held: brass rings set with cut blue crystal, each turning inside a ring of its own light and trailing star-sparks. The hood sits over a teal coat with a brown working harness across it — buckled straps, belt pouches, fingerless gloves, heavy boots — and a long red scarf that is the one hot colour on her. Behind her Bramblehollow: a village built up through a tree canopy on rope bridges and platforms, lanterns strung between them, waterfalls beyond, other Bramblekin going about their day. She is small in frame, which is a readability fact rather than a hitbox advantage — the gameplay rule is held in her implementation guards. Her whole anatomy must read as animal-like, **not** a human with fox ears.
...
**Visual language:** Longer-legged and more athletic than Mimzi, with powerful hare hind legs and feet, swept-back ears, short tail, weatherproof courier jacket, scarf, running wraps, map-and-letter satchel, and twin mechanical springbows. The **springbows** are brass and green-painted steel, exposed springs and limbs, carried one in each hand and ready to fire. Her jacket is **olive** with the hood up — a deliberate step from Mimzi's teal — her scarf is red and streams behind her, and her forearms and shins are wrapped for distance. She is caught mid-stride along a branch with her weight already forward, which is the whole character: she is moving. Behind her the Thistlewick routes — a village strung through the canopy on rope bridges, waterfalls falling past it, lanterns lit, other Bramblekin crossing. She is an experienced, fast-talking adult professional, competitive and impatient with delays, **not** another miniature mage: no floating trinkets, no spell light, **nothing** she carries is magical.
...
**Visual language:** Huge scarred, hunched reptilian silhouette with powerful digitigrade legs, muscular arms, a heavy tail, a chipped horn and broken horn stump, one clouded eye, torn frill, badly scarred snarl, and mismatched scorched mining armor. Thick **viscous black industrial slag** clings to the cleavers and armor gaps. Flesh and injuries remain visibly biological. The cleavers are hooked and oversized with their edges burning molten, and slag runs off them in ropes; more of it hangs from his jaw and pools in every seam of his armour. Behind him a working Ember Basin site: gantries and ore hoists over open lava channels, crews small on the walkways, banners hung from the scaffolding. He is a living animal: **never** a Forgeheart, a Fluxborn, or a transformed human. Rude, vicious, impatient, and terrifying. He still despises officials who knowingly send workers into lethal conditions — but that contempt stopped translating into protecting anyone a long time ago.
...
**Visual language:** A tall, unmistakably nonhuman figure made of shifting translucent air, pale mineral dust, drifting crystals, and flowing cloth; eyes glimmer through a face only suggested by currents. Chimes ring as she casts, and her form may loosen into a compact cyclone. **Brass tube chimes and small blue crystal pendants** hang from her on fine chains, and long white and teal cloth streams away in currents that keep moving after she stops, so her silhouette is horizontal and soft-edged, all ribbon and air. She floats, with the canyon crossings below her: bridges and terraces over cloud, waterfalls falling past floating stone, travellers small on the paths. She is soft cloth and cool air rather than faceted glass in warm gold, which is what keeps her separable from Oriel at distance. She is emergent weather with a mind: **never** a Fluxborn construct, and **never** a Merrin Echo. Gentle, attentive, quietly stubborn, and suddenly forceful when travelers are endangered.
...
**Origin:** Iron March freight and repair routes; **specific hometown not established**. **Nature:** Older human engineer with **extensive mechanical prostheses** — both arms from the shoulder and both legs below the knee; torso, neck and head are flesh. **Identity:** Medium-range physical engineer, deployable/utility specialist, prepared-position siege support. **Machine:** **Picket**, a single four-legged modular rivet-cannon-and-bulwark platform; equipment, **not** an aware companion or Fluxborn.

**Visual language:** Broad-shouldered older woman, short iron-gray hair, crooked healed nose, burned cheek, weathered red work coat, goggles pushed up on her forehead, and a tool harness over dark workwear. **Both arms are full mechanical prostheses from the shoulder** — banded brass-and-steel sleeves ending in articulated gripping hands — and both legs are mechanical from the knee down, with heavy plated knees and jointed feet. Her torso, neck and face are flesh, weathered and scarred; she carries rolled drawings under one arm. Picket has four heavy piston legs, a rotating rivet cannon, and a wide directional steel shield. Two visually distinct stationary modes: **Gun Platform OR Bulwark**, never simultaneous at full strength. Picket is equipment: **never** an aware companion, and **never** Fluxborn. Behind her the Iron March at work — a freight city in tiers, cranes and gantries, crews moving crates, red banners bearing a gear. Eudora is direct, practical, safety-minded, and deeply hostile to negligent employers.

**Lore:** Eudora repaired bridges, tunnels, engines, and freight infrastructure across the Iron March. A freight tunnel came down on a crew she was leading, and she came out of it without both arms and without both legs below the knee. She rebuilt herself from freight-grade parts — the tolerances were ones she already knew — and went back on the routes. **Picket began as mobile cover** for work crews endangered by falling debris or wildlife: she had spent a shift under a ceiling with nothing over her, and made certain the next crew was not. After a night-shift attack she fitted it with a heavy rivet cannon, which saved the crew but damaged the bridge they had just repaired. Newly reactivated ancient Flux systems threaten more routes, so she traveled toward the Crucible to investigate. She knows Raska, Kade, and Mavra through engineering work; she respects Relay and Torr as people, not salvage. Do **not** silently assign her Calder's Run as a birthplace.
