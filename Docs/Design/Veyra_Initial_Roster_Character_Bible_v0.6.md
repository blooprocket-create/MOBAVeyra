...
**Angeru's staging corrected from art (2026-09-21):** His approved art shows him **crouched on a rooftop under dramatic night light**, sword across his back. His paragraph specified the opposite staging — *"calmly and evenly lit, standing straight… **not** crouched"*, sword *"at the hip"* — so that staging is withdrawn and the description follows the art. **His characterisation is untouched.** The staging existed to make him read as methodical rather than as a generic edgy assassin, and what does that job in the art is his **face**: flat, level, no snarl, uncovered. The guardrail is rebuilt around the face instead of the pose, so it still forbids the wrong reading. The antagonist direction in `render_sheet.py` loses only its staging clause; *composed, methodical, unhurried, explains himself, takes no pleasure* all stand. The art is **silent** on the cut-out insignia rather than contradicting it, so that detail stays — canon calls it the character. No kit, lore, role, region or roster count changed.  
