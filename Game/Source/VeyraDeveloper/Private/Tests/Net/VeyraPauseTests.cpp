...
namespace VeyraNetTests
{
	...
	// runs on one of the mechanisms checked here: effect durations (cooldowns, buffs, shields),
	// world timers (respawn, buyback, spawn and penalty clocks), world time (the match clock) and
	// actor ticks (movement, regeneration, combat).
	//
	...
	NETWORK_TEST_CLASS(MatchPause, "Veyra.Net")
	{
		...
		TEST_METHOD(PauseFreezesGameplayClocksButNotTheNetwork)
		{
			StartMatch(Network, Layout, EVeyraMatchPhase::Live)
				.ThenServer(TEXT("Start a shield, a timer and a move"), [this](FState& State) {
				...
				.UntilServer(TEXT("The shield and the timer end on their frozen schedule"), [this](FState& State) {
					return FindShield(State, Shield) == nullptr && bTimerFired;
				})
				.ThenServer(TEXT("They resumed from their saved values"), [this](FState& State) {
					ASSERT_THAT(IsTrue(State.World->GetTimeSeconds() >= PausedWorldTime + FMath::Max(PausedShieldRemaining, PausedTimerRemaining)));
					ASSERT_THAT(IsTrue(MoverState(State)->GetPawn()->GetActorLocation() != PausedLocation));
				})
